...
from dataset_parser import DatasetBuilder
...
class ModelTrainer:
    def __init__(self, logger=Log(print), embedding_file='data/wiki-news-300d-1M.vec',
                 ...
                 validation_split=0.33, test_split=0.2, batch_size=50, epochs=25, embedding_dim=100,
                 additional_feats_enabled=True):

        self.dataset = DatasetBuilder(logger)
        self.logger = logger
    ...
    def train_model(self):
        ...
        # start fitting model
        self.model.fit([np.array(q_train), np.array(d_train), np.array(addn_feat_train)],
                       np.array(y_train),
                       epochs=self.epochs,
        ...
        self.logger.write_log(f'Start evaluating our model', 'evaluate')

        # evaluate the model with test-set
        result = self.model.evaluate([np.array(q_test), np.array(d_test), np.array(addn_feat_test)], np.array(y_test), verbose=1)
    ...
    def save_model(self, model_name):
        ...
        full_path = os.path.join('output', model_name + '.h5')
